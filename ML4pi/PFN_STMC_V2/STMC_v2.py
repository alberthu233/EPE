...
## GEOMETRY DICTIONARY ##
geo_file = ur.open(ext_datapath + fileNames[0])
...
X_large = np.lib.format.open_memmap(stmc_v2_path+'X_large.npy', mode='w+', dtype=np.float64,
                       shape=(2500000,1500,6), fortran_order=False, version=None)
Y_large = np.lib.format.open_memmap(stmc_v2_path+'Y_large.npy', mode='w+', dtype=np.float64,
...
t0 = t.time()
X = np.lib.format.open_memmap(stmc_v2_path+'STMC_v2_X_'+str(Nfile)+'_files.npy',
...
Y = np.lib.format.open_memmap(stmc_v2_path+'STMC_v2_Y_'+str(Nfile)+'_files.npy',
                             mode='w+', dtype=np.float64, shape=(tot_nEvts, 3))
np.copyto(dst=Y, src=Y_large[:tot_nEvts,:], casting='same_kind', where=True)
del Y_large

np.save(stmc_v2_path+'Eta_v2_'+str(Nfile)+'_files', Eta_large[:tot_nEvts])

t1 = t.time()
print()
print('Time to copy new and delete old: '+str(t1-t0)+' (s)')
print()
os.system('rm '+stmc_v2_path+'Y_large.npy')
os.system('rm '+stmc_v2_path+'X_large.npy')
# t0 = t.time()
